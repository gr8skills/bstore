...
class install extends Command
{
    ...
    /**
    *  Checking .env2 file and if not found then create .env2 file.
    *  Then ask for database name, password & username to set
    *  On .env2 file so that we can easily migrate to our db
    */
    public function checkForEnvFile()
    {
        $envExists = File::exists(base_path() . '/.env2');
        if (!$envExists) {
    ...
    /**
     * Create a new .env2 file.
     */
    public function createEnvFile()
    {
        try {
            File::copy('.env2.example', '.env2');
            ...
            $this->error('Error in creating .env2 file, please create it manually and then run `php artisan migrate` again.');
        }
    }

    /**
     * Add the database credentials to the .env2 file.
     */
